...
def Haas(dataset, model, cam_instance):
    ...
    for image, label in dataset:
        ...
        # get pixel attributions for the real label class ('heatmap' as referenced in the paper)
        heatmap_target = [ClassifierOutputTarget(label)]
        pixel_attributions = cam_instance(input_tensor=image.unsqueeze(0), targets=heatmap_target)[0,:,:]
