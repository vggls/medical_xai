...
def get_explanation(X, label, model, cam_instance):
    
    targets = [ClassifierOutputTarget(label)]
    pixel_attributions = cam_instance(input_tensor=X, targets=targets)
...
def get_exp_sens(X, label, model, expl, cam_instance, radius, iterations):
    ...
    for _ in range(iterations):
        ...
        expl_eps = get_explanation(X_noisy, label, model, cam_instance)

        #max_diff = max(max_diff, np.linalg.norm(expl-expl_eps)/norm)  # by default this is the Frobenius norm
        # we consider the explanations difference because we want to see how much the CLASS attribution map changes
...
def MaxSensitivity_Dataset(dataset, model, cam_instance, radius, iterations):
    ...
    for image, label in dataset:
            ...
            if real_label == pred_label:
                ...
                expl = get_explanation(image.unsqueeze(0), real_label, model, cam_instance)
                score = get_exp_sens(image.unsqueeze(0), real_label, model, expl, cam_instance, radius, iterations)
