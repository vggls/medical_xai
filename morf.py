...
def AOPC_Dataset(dataloader, model, xai_algorithm, noise, target_layer=None, plot_aopc_per_step=False):
    ...
    for batch_images, batch_labels in dataloader:
        for image, label in zip(batch_images, batch_labels):
            ...
            if real_label == pred_label:
                ...
                diffs, score = MoRF(image.unsqueeze(0), regions, model, noise).aopc()
                differences.append(diffs) #for the plot
                scores.append(score) #for total aopc score
    ...
    return round(dataset_aopc_score, 2)